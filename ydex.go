...
// StructsExecRet is an auto generated low-level Go binding around an user-defined struct.
type StructsExecRet struct {
	Success bool
	Err     string
}
...
// ExecuteTrades is a paid mutator transaction binding the contract method 0x84ae6c6c.
//
// Solidity: function executeTrades((uint8,uint128,address,uint8,uint8,uint64,bool,uint64,uint64,string,uint8,uint8,uint64,bytes)[] buys, (uint8,uint128,address,uint8,uint8,uint64,bool,uint64,uint64,string,uint8,uint8,uint64,bytes)[] sells, (string,string,address,address,uint64,uint64,uint64,uint64,address,address,uint64,uint64,uint64,uint8)[] trades) returns()
func (_Ydexabi *YdexabiTransactor) ExecuteTrades(opts *bind.TransactOpts, buys []StructsOrder, sells []StructsOrder, trades []StructsTrade) (*types.Transaction, error) {
	return _Ydexabi.contract.Transact(opts, "executeTrades", buys, sells, trades)
}

// ExecuteTrades is a paid mutator transaction binding the contract method 0x84ae6c6c.
//
// Solidity: function executeTrades((uint8,uint128,address,uint8,uint8,uint64,bool,uint64,uint64,string,uint8,uint8,uint64,bytes)[] buys, (uint8,uint128,address,uint8,uint8,uint64,bool,uint64,uint64,string,uint8,uint8,uint64,bytes)[] sells, (string,string,address,address,uint64,uint64,uint64,uint64,address,address,uint64,uint64,uint64,uint8)[] trades) returns()
func (_Ydexabi *YdexabiSession) ExecuteTrades(buys []StructsOrder, sells []StructsOrder, trades []StructsTrade) (*types.Transaction, error) {
	return _Ydexabi.Contract.ExecuteTrades(&_Ydexabi.TransactOpts, buys, sells, trades)
}

// ExecuteTrades is a paid mutator transaction binding the contract method 0x84ae6c6c.
//
// Solidity: function executeTrades((uint8,uint128,address,uint8,uint8,uint64,bool,uint64,uint64,string,uint8,uint8,uint64,bytes)[] buys, (uint8,uint128,address,uint8,uint8,uint64,bool,uint64,uint64,string,uint8,uint8,uint64,bytes)[] sells, (string,string,address,address,uint64,uint64,uint64,uint64,address,address,uint64,uint64,uint64,uint8)[] trades) returns()
func (_Ydexabi *YdexabiTransactorSession) ExecuteTrades(buys []StructsOrder, sells []StructsOrder, trades []StructsTrade) (*types.Transaction, error) {
	return _Ydexabi.Contract.ExecuteTrades(&_Ydexabi.TransactOpts, buys, sells, trades)
}
...
// YdexabiTradesExecutedIterator is returned from FilterTradesExecuted and is used to iterate over the raw logs and unpacked data for TradesExecuted events raised by the Ydexabi contract.
type YdexabiTradesExecutedIterator struct {
	Event *YdexabiTradesExecuted // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *YdexabiTradesExecutedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(YdexabiTradesExecuted)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *YdexabiTradesExecutedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *YdexabiTradesExecutedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// YdexabiTradesExecuted represents a TradesExecuted event raised by the Ydexabi contract.
type YdexabiTradesExecuted struct {
	Vals []StructsExecRet
	Raw  types.Log // Blockchain specific contextual infos
}

// FilterTradesExecuted is a free log retrieval operation binding the contract event 0xc2820e5d73bdfe3136d93ac2f85800079f6ca95b2d23046b6709a98a472dd3a5.
//
// Solidity: event TradesExecuted((bool,string)[] vals)
func (_Ydexabi *YdexabiFilterer) FilterTradesExecuted(opts *bind.FilterOpts) (*YdexabiTradesExecutedIterator, error) {

	logs, sub, err := _Ydexabi.contract.FilterLogs(opts, "TradesExecuted")
	if err != nil {
		return nil, err
	}
	return &YdexabiTradesExecutedIterator{contract: _Ydexabi.contract, event: "TradesExecuted", logs: logs, sub: sub}, nil
}

// WatchTradesExecuted is a free log subscription operation binding the contract event 0xc2820e5d73bdfe3136d93ac2f85800079f6ca95b2d23046b6709a98a472dd3a5.
//
// Solidity: event TradesExecuted((bool,string)[] vals)
func (_Ydexabi *YdexabiFilterer) WatchTradesExecuted(opts *bind.WatchOpts, sink chan<- *YdexabiTradesExecuted) (event.Subscription, error) {

	logs, sub, err := _Ydexabi.contract.WatchLogs(opts, "TradesExecuted")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(YdexabiTradesExecuted)
				if err := _Ydexabi.contract.UnpackLog(event, "TradesExecuted", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseTradesExecuted is a log parse operation binding the contract event 0xc2820e5d73bdfe3136d93ac2f85800079f6ca95b2d23046b6709a98a472dd3a5.
//
// Solidity: event TradesExecuted((bool,string)[] vals)
func (_Ydexabi *YdexabiFilterer) ParseTradesExecuted(log types.Log) (*YdexabiTradesExecuted, error) {
	event := new(YdexabiTradesExecuted)
	if err := _Ydexabi.contract.UnpackLog(event, "TradesExecuted", log); err != nil {
		return nil, err
	}
